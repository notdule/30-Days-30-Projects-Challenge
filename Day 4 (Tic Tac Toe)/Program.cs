...
bool CheckVictory()
{
    ...
    bool row1 = grid[6] == grid[7] && grid[7] == grid[8];
    bool collumn1 = grid[0] == grid[3] && grid[3] == grid[6];
    bool collumn2 = grid[1] == grid[4] && grid[4] == grid[7];
    bool collumn3 = grid[2] == grid[5] && grid[5] == grid[8];
    bool diagDown = grid[0] == grid[4] && grid[4] == grid[8];
    bool diagUp = grid[6] == grid[4] && grid[4] == grid[2];

    return row1 || row2 || row3 || collumn1 || collumn2 || collumn3 || diagDown || diagUp;
}
